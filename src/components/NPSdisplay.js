...
const NPSdisplay = ({ nps }) => {
    ...
    const [npsResult] = useState([
        { title: " Excelência", avatar: smile , text: "São as empresas que satisfazem seus clientes, atendendo às suas expectativas e garantindo uma boa experiência ao usuário, conquistando consumidores fiéis que recomendam e defendem a marca.", color: "#2dc937" },
        { title: "Qualidade", avatar: smile , text: "Empresas qualificadas nessa zona contam com clientes satisfeitos com a marca, mas que ressaltaram pontos que deixaram a desejar. Realizar uma avaliação da jornada do cliente é importante para que a empresa possa identificar possíveis falhas.", color: "#99c140" },
                ...
                <img src={nps >= 75 ? npsResult[0].avatar :
                    nps >= 50 ? npsResult[1].avatar :
                        nps >= 0 ? npsResult[2].avatar :
                            ...
                            <h3>zona de</h3>
                <div className='NPS-display-title-text'> <br/> <h1>{nps >= 75 ? npsResult[0].title :
                    nps >= 50 ? npsResult[1].title :
                        nps >= 0 ? npsResult[2].title :
                            nps >= -100 ? npsResult[3].title : "Esperando inserção de dados"}</h1>
                </div>
            </div>
            <hr />
